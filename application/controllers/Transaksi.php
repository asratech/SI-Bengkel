...
class Transaksi extends CI_Controller {
            ...
            if ($bayar >= $jumlah) {
                ...
                if ($this->Model_transaksi->createTransaksi($data)) {
                    ...
                    foreach ($tmpSubTransaksiJasa->result() as $value) {
                        if($value->id_jasa == 2 || $value->id_jasa == 3){
    ...
    public function Faktur($id){
        $data = array(
            "title" => "Faktur",
            "transaksi" => $this->Model_transaksi->whereViewTransaksi($id)->result(),
            "transaksi_barang" => $this->Model_transaksi->whereViewTransaksiBarangByIdTransaksi($id)->result(),
            "transaksi_jasa" => $this->Model_transaksi->whereViewTransaksiJasaByIdTransaksi($id)->result(),
        );
    ...
    {
        $awal = date_format(date_create($this->input->post('awal')), 'Y-m-d h:i:s');
        $akhir = date_format(date_create($this->input->post('akhir')), 'Y-m-d h:i:s');
        $data = array(
            "title" => "Laporan Transaksi",
            "transaksi" => $this->Model_transaksi->reportTransaksi($awal, $akhir)->result(),
        );
        $html=$this->load->view('Laporan/laporan_transaksi.php', $data, true);
        $pdfFilePath = "LapotanTransaksi_".$awal."-".$akhir.".pdf";
        $this->load->library('m_pdf');
        $pdf = $this->m_pdf->load();
        $pdf->WriteHTML($html);
        $pdf->Output($pdfFilePath, "I");
